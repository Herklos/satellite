...
describe("push", () => {
  ...
  it("skipTimestamps stores empty timestamps object", async () => {
    const store = new MemoryObjectStore()
    const result = await push(store, "col/doc1", { _encrypted: "blob" }, null, undefined, true)
    expect(result.ok).toBe(true)
  ...
  it("skipTimestamps works on subsequent pushes", async () => {
    const store = new MemoryObjectStore()
    const r1 = await push(store, "col/doc1", { _encrypted: "v1" }, null, undefined, true)
    if (!r1.ok) throw new Error("first push failed")

    const r2 = await push(store, "col/doc1", { _encrypted: "v2" }, r1.hash, undefined, true)
    expect(r2.ok).toBe(true)

    const raw = await store.getString("col/doc1")
    const doc = JSON.parse(raw!) as StoredDocument
    expect(doc.timestamps).toEqual({})
    expect(doc.data).toEqual({ _encrypted: "v2" })
  })
